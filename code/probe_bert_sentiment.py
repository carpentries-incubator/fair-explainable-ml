...
# Load the IMDB Movie Review dataset. The dataset contains text reviews and their corresponding sentiment labels (positive or negative).
# The label 1 corresponds to a positive review, and 0 corresponds to a negative review.
train_dataset, dev_dataset, test_dataset = load_imdb_dataset(keep_samples=50)

# Load a model from huggingface. We will use this model to get the embeddings for the probe.
...
# The 'base' size of the model has 12 layers, 768 hidden dimensions, and 12 attention heads (110 million parameters)!
# 'uncased' means that the model is case-insensitive, and the vocabulary is lowercased.
model, tokenizer = load_model('bert-base-uncased')  # Give a list of acceptable models
...
# Let's see which layer has most information about sentiment analysis on IMDB.
